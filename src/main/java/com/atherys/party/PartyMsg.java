...
public class PartyMsg {
    ...
    /**
     * Sends an info message to the given user. Uses {@link User#isOnline()} to determine if the user
     * is online first. If so, the message will be delivered, else not.
     *
    ...
    public static void info(User user, Object... msg) {
        if (user.isOnline() && user.getPlayer().isPresent()) {
            info(user.getPlayer().get(), msg);
        }
    }

    /**
     * Sends an error message to the given user. Uses {@link User#isOnline()} to determine if the user
     * is online first. If so, the message will be delivered, else not.
     *
     * @param user The user to whom the message will be sent.
     * @param msg  The message. Will later be wrapped in a {@link Text} object.
